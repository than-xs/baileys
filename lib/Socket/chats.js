...
const makeChatsSocket = (config) => {
    ...
    try {
        ...
        console.error("========== CHECK BAN ERROR ==========");
        
        if (err?.reason === 'old_version') {
            console.error("Error: Baileys/WA Version sudah usang. Update Baileys atau library version-nya.");
        }
        ...
        let resultData = {
            ...
            isNeedOfficialWa: false,
            number: jid,
            error: err?.reason || 'unknown_error'
        };
      
        const errorData = err?.data || err;

        if (errorData?.appeal_token || errorData?.reason === 'banned') {
            resultData.isBanned = true;
            resultData.data = {
                violation_type: errorData.violation_type || null,
                appeal_token: errorData.appeal_token || null,
            };
        } 

        else if (errorData?.reason === 'bad_parameters' || errorData?.reason === 'blocked' || errorData?.status === 'fail') {

            if (errorData?.reason !== 'old_version') {
