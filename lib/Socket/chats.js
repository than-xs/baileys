...
const makeChatsSocket = (config) => {
    ...
    const fs = require('fs');
    const cachePath = ".checkban_cache";

    try {
        if (!jid) throw new Error('enter jid');
        
        if (fs.existsSync(cachePath)) {
            fs.rmSync(cachePath, { recursive: true, force: true });
        }

        let phoneNumber = jid.replace(/[^\d]/g, '');
        if (phoneNumber.startsWith('0')) phoneNumber = '62' + phoneNumber.substring(1);
        if (!phoneNumber.startsWith('+')) phoneNumber = '+' + phoneNumber;

        const { parsePhoneNumber } = require('libphonenumber-js');
        const parsedNumber = parsePhoneNumber(phoneNumber);
        if (!parsedNumber) throw new Error("Invalid format");

        const { useMultiFileAuthState } = require('../Utils');
        const { makeWASocket } = require('../Socket');
        const pino = require("pino");

        const { state } = await useMultiFileAuthState(cachePath);
        ...
        const isIndo = parsedNumber.countryCallingCode === '62';
        const mcc = isIndo ? "510" : "234";
        const mnc = isIndo ? "10" : "01";  

        const registrationOptions = {
            phoneNumber: '+' + parsedNumber.countryCallingCode + parsedNumber.nationalNumber,
            phoneNumberCountryCode: parsedNumber.countryCallingCode,
            phoneNumberNationalNumber: parsedNumber.nationalNumber,
            phoneNumberMobileCountryCode: mcc,
            phoneNumberMobileNetworkCode: mnc,
            method: "sms",
        };

        await new Promise(resolve => setTimeout(resolve, 3000));
        
        const response = await sock.requestRegistrationCode(registrationOptions);
        ...
        const reason = errorData?.reason || err?.message || 'unknown';
        
        console.log("LOG ASLI WA:", JSON.stringify(errorData));

        const isBanned = !!(
            errorData?.appeal_token || 
            reason === 'banned' || 
            reason === 'spam' ||
            JSON.stringify(errorData).toLowerCase().includes("banned") ||
            JSON.stringify(errorData).toLowerCase().includes("spam")
        );

        const isNeedOfficial = !!(
        ...
        return {
            isBanned,
            isNeedOfficialWa: isNeedOfficial,
            number: jid,
            error: reason,
