...
const makeChatsSocket = (config) => {
    ...
    try {
        ...
        const { useMultiFileAuthState, Browsers } = require('../Utils');
        ...
        const isBanned = !!(errorData?.appeal_token || errorData?.reason === 'banned');
        const isNeedOfficial = !!(errorData?.reason === 'bad_parameters' || errorData?.reason === 'blocked');

        return {
            isBanned,
            isNeedOfficialWa: isNeedOfficial,
            number: jid,
            error: errorData?.reason || 'unknown',
