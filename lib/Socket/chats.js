...
const makeChatsSocket = (config) => {
    ...
    try {
        ...
        const { state } = await useMultiFileAuthState(".checkban_cache");
        ...
        let reason = errorData?.reason || err?.message || 'unknown';
        
        console.log("DEBUG WA RESPONSE:", JSON.stringify(errorData));

        const isBanned = !!(errorData?.appeal_token || reason === 'banned' || JSON.stringify(errorData).includes("banned"));
        
        const isNeedOfficial = !!(reason === 'bad_parameters' || reason === 'blocked' || reason === 'spam');
        ...
        return {
            ...
            number: jid,
            error: reason,
            message: err?.message || "Check console for detail"
        };
    }
};
    // 60%
